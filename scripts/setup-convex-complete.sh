...
if npx convex dev --configure; then
    ...
    if [ -f ".convex/config.json" ]; then
        ...
        if [ -n "$DEPLOYMENT_URL" ]; then
            ...
            # Extract deployment name from URL (e.g., "standing-marlin-7" from "https://standing-marlin-7.convex.cloud")
            DEPLOYMENT_NAME=$(echo "$DEPLOYMENT_URL" | sed 's|https://\([^.]*\)\.convex\.cloud|\1|')
            if [ -n "$DEPLOYMENT_NAME" ]; then
                sed -i "s|CONVEX_DEPLOYMENT=.*|CONVEX_DEPLOYMENT=$DEPLOYMENT_NAME|" .env.local
                echo "✅ Updated .env.local with deployment: $DEPLOYMENT_NAME"
            fi
        fi
    fi
